...
host.defineController("Korg", "J28 nanoKONTROL 2 Current", "1.0", "e17f6293-39bf-4160-b36c-65df38c0f72f");
...
function init()
{
	...
	// cursorDevice = host.getCursorDevice ();
	...
	var banana = cursorDevice.createCursorRemoteControlsPage(8);

 //   cursorDevice.CursorRemoteControlsPage(7, function(page)
	// {
	// 	paramPage = page;
	// });
...
function allIndicationsOff()
{
  ...
  //     cursorDevice.getMacro(p).getAmount().setIndication(false);
...
{
   // isSetPressed ? cursorDevice.getParameter(index).reset() : cursorDevice.getParameter(index).set(val, 128);
};
